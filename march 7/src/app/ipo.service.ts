...
export class IpoService {
  httpUrl='http://localhost:8004/ipos/';
  ...
  deleteIPO(id:number):Observable<IPO>{
    return this.ht.delete(`http://localhost:8004/ipos/${id}`);
  }
  updateIPOInfo(ipo :IPO):Observable<IPO>{
    return this.ht.put(`http://localhost:8004/ipos` , ipo);
  }
  getIPOById(id :number):Observable<IPO>{
    return this.ht.get(`http://localhost:8004/ipos/${id}`);
  }
}